...
\]
for \(s > 5/2\). This establishes linking growth on a large open set in the Baire-category sense \cite{ArnoldKhesin1998}.
...
\]
Because any nonzero swirl immediately produces \(\mathcal{H}_{\rm top} \gtrsim t \|\omega_\theta\|_2^2\), the nonlinear term is integrable over \([0,\infty)\). The Ladyzhenskaya--Prodi--Serrin criterion then yields global boundedness in \(H^s\) for \(s > 5/2\), hence global smooth solutions \cite{Ladyzhenskaya1967}.
\end{proof}
